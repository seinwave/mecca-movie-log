...
class StaticPagesController < ApplicationController
  ...
  def stats
    ...
    @matt_score_on_divergence = convert_to_letter_grade(Rating.where(movie_id: @biggest_divergence.id,
                                                                     user_id: matt.id).first)
    @reba_score_on_divergence = convert_to_letter_grade(Rating.where(movie_id: @biggest_divergence.id,
                                                                     user_id: reba.id).first)
  end
end
